...
export function CreatorAgentLayout({ children }: { children: ReactNode }) {
  ...
  >({});
  const [detailSelectedVariantId, setDetailSelectedVariantId] =
  ...
  useEffect(() => {
    ...
    if (detailProduct.variants && detailProduct.variants.length > 0) {
      ...
      setDetailSelectedVariantId(first.id);
      ...
      setDetailSelectedVariantId(null);
    }

    setDetailSelectedOptions(next);
    setDetailActiveImageIndex(0);
  }, [detailProduct?.id]);

  const detailSelectedVariant = useMemo(() => {
    if (!detailProduct?.variants || detailProduct.variants.length === 0) {
      return null;
    }

    if (detailSelectedVariantId) {
      const byId = detailProduct.variants.find(
        (v) => v.id === detailSelectedVariantId,
      );
      if (byId) return byId;
    }

    const entries = Object.entries(detailSelectedOptions).filter(
      ([, v]) => typeof v === "string" && v,
    );
    if (!entries.length) {
  ...
  }, [detailProduct, detailSelectedOptions, detailSelectedVariantId]);
        ...
        <div className="flex flex-1 flex-col bg-[#fffefc] lg:flex-row lg:overflow-hidden">
          ...
          <section className="flex flex-1 flex-col bg-[#fffefc] px-4 py-4 lg:px-8">
            <div className="flex min-h-0 flex-1 flex-col overflow-y-auto">
              {detailProduct ? (
                <div className="mx-auto flex w-full max-w-5xl flex-col gap-4 rounded-3xl bg-[#fffaf5] p-4 shadow-md sm:p-6">
                  <div className="flex flex-col gap-6 md:flex-row">
                    {detailImages.length > 0 && (
                      <div className="w-full bg-[#f5e3d4] md:w-1/2">
                    ...
                    <div className="flex flex-1 flex-col gap-4 text-[13px] text-[#3f3125]">
                        ...
                        return (
                          <div className="space-y-2 text-[13px]">
                            ...
                            <div className="text-2xl font-semibold">
                              {detailProduct.currency} {price.toFixed(2)}
                            </div>
                                ...
                                <div key={option.name}>
                                  ...
                                  <div className="mt-1 flex flex-wrap gap-2">
                                    {option.values.map((value) => {
                                      const isSelected =
                                        detailSelectedOptions[option.name] ===
                                        value;
                                      return (
                                        <button
                                          key={value}
                                          ...
                                          onClick={() => {
                                            ...
                                            if (
                                              ...
                                              setDetailSelectedVariantId(
                                                match.id,
                                              );
                                              if (match.imageUrl) {
                                                const idx =
                                                  detailImages.findIndex(
                                                    (url) =>
                                                      url === match.imageUrl,
                                                  );
                                                if (idx >= 0) {
            ...
            >
              <div className="flex h-full flex-col overflow-y-auto sm:flex-row">
                ...
                <div className="flex flex-1 flex-col gap-4 p-4 sm:p-6">
                  ...
                  {(() => {
                    const price =
                      (detailSelectedVariant && detailSelectedVariant.price) ||
                      detailProduct.price;
                    return (
                      <div className="space-y-2 text-[13px]">
                        <div className="text-[11px] font-medium uppercase tracking-wide text-[#a38b78]">
                                      ...
                                      onClick={() => {
                                        ...
                                        if (
                                          ...
                                          const match =
                                            ...
                                            detailProduct.variants[0];
                                          setDetailSelectedVariantId(match.id);
                                          if (match.imageUrl) {
                                            const idx = detailImages.findIndex(
                                              (url) => url === match.imageUrl,
                                            );
                                            if (idx >= 0) {
                                              setDetailActiveImageIndex(idx);
                                            }
                                          }
                                        }
                                      }}
